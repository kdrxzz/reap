...
const ADMIN_PASSWORD = 'admin'; // Only 5 chars - that's OK for admin
...
// API Routes - FIXED: Moved admin login check to beginning
app.post('/api/register', async (req, res) => {
    try {
        ...
        // SKIP password length validation for admin email
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        // FIXED: Check admin login FIRST without password length validation
        if (email === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
            const adminToken = Buffer.from(`${ADMIN_EMAIL}:${Date.now()}:admin`).toString('base64');
            console.log(`✅ Admin login successful for ${ADMIN_EMAIL}`);
        ...
        // For regular users, check password length (only if not admin)
...
╚═══════════════════════════════════════════════════╝
    ...
    const adminToken = Buffer.from(`${ADMIN_EMAIL}:${Date.now()}:admin`).toString('base64');
    console.log(`🔑 Admin token (for testing): ${adminToken}`);
    console.log(`✅ Admin login: ${ADMIN_EMAIL} with password "${ADMIN_PASSWORD}" (${ADMIN_PASSWORD.length} chars)`);
});